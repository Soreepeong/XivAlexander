...
HWND XivAlexander::Apps::MainApp::Window::BaseWindow::InternalCreateWindow(
...
std::set<const XivAlexander::Apps::MainApp::Window::BaseWindow*> s_allBaseWindows;

XivAlexander::Apps::MainApp::Window::BaseWindow::BaseWindow(const WNDCLASSEXW& wndclassex,
	_In_opt_ LPCWSTR lpWindowName,
	_In_ DWORD dwStyle,
	_In_ DWORD dwExStyle,
	_In_ int X,
...
XivAlexander::Apps::MainApp::Window::BaseWindow::~BaseWindow() {
	Destroy();
	UnregisterClassW(m_windowClass.lpszClassName, Dll::Module());
}

const std::set<const XivAlexander::Apps::MainApp::Window::BaseWindow*>& XivAlexander::Apps::MainApp::Window::BaseWindow::All() {
	return s_allBaseWindows;
}

bool XivAlexander::Apps::MainApp::Window::BaseWindow::IsDialogLike() const {
	return false;
}

LRESULT XivAlexander::Apps::MainApp::Window::BaseWindow::RunOnUiThreadWait(const std::function<LRESULT()>& fn) {
	return SendMessageW(m_hWnd, AppMessageRunOnUiThread, 0, reinterpret_cast<LPARAM>(&fn));
}

auto XivAlexander::Apps::MainApp::Window::BaseWindow::RunOnUiThread(std::function<void()> fn, bool immediateIfNoWindow) -> bool {
	if (!m_hWnd) {
...
double XivAlexander::Apps::MainApp::Window::BaseWindow::GetZoom() const {
	try {
...
LRESULT XivAlexander::Apps::MainApp::Window::BaseWindow::OnSysCommand(WPARAM commandId, short xPos, short yPos) {
	return DefWindowProcW(m_hWnd, WM_SYSCOMMAND, commandId, MAKELPARAM(xPos, yPos));
}

void XivAlexander::Apps::MainApp::Window::BaseWindow::OnDestroy() {
	OnDestroyListener();
...
void XivAlexander::Apps::MainApp::Window::BaseWindow::Destroy() {
	if (m_bDestroyed)
		return;
	m_bDestroyed = true;
...
xivres::util::on_dtor XivAlexander::Apps::MainApp::Window::BaseWindow::WithTemporaryFocus() const {
