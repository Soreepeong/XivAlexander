...
struct XivAlexander::Apps::MainApp::Features::PatchCode::Implementation {
	App& App;
		...
		for (auto& p : Config->Game.PatchCode.Value()) {
			...
			try {
				...
				for (const auto& instruction : searchInstructions) {
					if (instruction.at(0) == "find") {
					...
					} else if (instruction.at(0) == "resolve_branch" || instruction.at(0) == "resolve_ip") {
						...
						if ((inst.meta.category == ZYDIS_CATEGORY_UNCOND_BR || inst.meta.category == ZYDIS_CATEGORY_COND_BR || inst.meta.category == ZYDIS_CATEGORY_CALL) && operands[0].type == ZYDIS_OPERAND_TYPE_IMMEDIATE) {
							if (uint64_t resultAddress; ZYAN_STATUS_SUCCESS == ZydisCalcAbsoluteAddress(&inst, &operands[0], reinterpret_cast<size_t>(pointer), &resultAddress)) {
								pointer = reinterpret_cast<char*>(resultAddress);
							} else {
					...
					} else if (instruction.at(0) == "resolve_memory" || instruction.at(0) == "resolve_rip") {
						ZydisDecodedInstruction inst;
						ZydisDecodedOperand operands[ZYDIS_MAX_OPERAND_COUNT];
						if (IsBadReadPtr(pointer, 16))
							throw std::runtime_error(std::format("0x{:X}: bad read ptr", reinterpret_cast<size_t>(pointer)));
						...
						for (ZyanU8 i = 0; i < inst.operand_count; i++) {
							if (operands[i].type != ZYDIS_OPERAND_TYPE_MEMORY)
								continue;
							if (uint64_t resultAddress; ZYAN_STATUS_SUCCESS == ZydisCalcAbsoluteAddress(&inst, &operands[i], reinterpret_cast<size_t>(pointer), &resultAddress)) {
								pointer = reinterpret_cast<char*>(resultAddress);
								resolved = true;
								break;
							}
						...
						if (!resolved)
							throw std::runtime_error(std::format("0x{:X}: no rip-relative memory operand", reinterpret_cast<size_t>(pointer)));

					} else if (instruction.at(0) == "write") {
						std::vector<char> patchBytes;
						for (size_t i = 1; i < instruction.size(); i++) {
							for (const auto& part : xivres::util::split(instruction[i], std::string(" "))) {
								if (part.empty())
									continue;
...
XivAlexander::Apps::MainApp::Features::PatchCode::PatchCode(App& app)
	: m_pImpl(std::make_unique<Implementation>(app)) {
}

XivAlexander::Apps::MainApp::Features::PatchCode::~PatchCode() = default;
