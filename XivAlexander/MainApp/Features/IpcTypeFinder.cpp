...
struct XivAlexander::Apps::MainApp::Features::IpcTypeFinder::Implementation {
	...
	public:
		...
		SingleConnectionHandler(Implementation& pImpl, SingleConnection& conn)
			...
			conn.AddIncomingFFXIVMessageHandler(this, [&](auto pMessage) {
				if (pMessage->Type == message_type::Ipc && pMessage->Data.Ipc.Type == ipc_type::InterestedType) {
					if (pMessage->CurrentActor == pMessage->SourceActor) {
						...
						} else if (pMessage->Length == sizeof(message_header) + sizeof(ipc_header) + sizeof(ipcs::S2C_ActorControlSelf)) {
						...
						} else if (pMessage->Length == sizeof(message_header) + sizeof(ipc_header) + sizeof(ipcs::S2C_ActorCast)) {
							Impl.Logger->Format(
								LogCategory::IpcTypeFinder,
			...
			conn.AddOutgoingFFXIVMessageHandler(this, [&](auto pMessage) {
				if (pMessage->Type == message_type::Ipc && pMessage->Data.Ipc.Type == ipc_type::InterestedType) {
					if (pMessage->Length == sizeof(ipcs::C2S_ActionRequest)) {
						// Test ActionRequest
					...
					if (pMessage->Length == sizeof(ipcs::C2S_ActionRequestGroundTargeted)) {
						// Test ActionRequest
						const auto& actionRequest = pMessage->Data.Ipc.Data.C2S_ActionRequest;
						Impl.Logger->Format(
							LogCategory::IpcTypeFinder,
...
XivAlexander::Apps::MainApp::Features::IpcTypeFinder::IpcTypeFinder(App& app)
	: m_pImpl(std::make_unique<Implementation>(app)) {
}

XivAlexander::Apps::MainApp::Features::IpcTypeFinder::~IpcTypeFinder() = default;
