...
static std::vector<uint8_t> CreateHeaderForNonCompressedBinaryEntryProvider(size_t size) {
	...
	const auto headerAlignment = Align(sizeof FileEntryHeader + blockAlignment.Count * sizeof BlockHeaderLocator);
...
Sqex::Sqpack::OnTheFlyBinaryEntryProvider::OnTheFlyBinaryEntryProvider(EntryPathSpec pathSpec, std::filesystem::path path, bool openImmediately, int compressionLevel)
...
Sqex::Sqpack::OnTheFlyBinaryEntryProvider::OnTheFlyBinaryEntryProvider(EntryPathSpec pathSpec, std::shared_ptr<const RandomAccessStream> stream, int compressionLevel)
...
uint64_t Sqex::Sqpack::OnTheFlyBinaryEntryProvider::MaxPossibleStreamSize() const {
	return reinterpret_cast<const FileEntryHeader*>(&m_header[0])->GetTotalEntrySize();
}
...
uint64_t Sqex::Sqpack::OnTheFlyBinaryEntryProvider::ReadStreamPartial(const RandomAccessStream& stream, uint64_t offset, void* buf, uint64_t length) const {
	...
	const auto& header = *reinterpret_cast<const FileEntryHeader*>(&m_header[0]);
