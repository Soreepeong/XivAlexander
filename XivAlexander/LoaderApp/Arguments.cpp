...
XivAlexander::LoaderApp::Arguments::Arguments()
	...
	argp.add_argument("--action", "-a")
		.help(xivres::util::unicode::convert<std::string>(FindStringResourceEx(Module(), IDS_HELP_ACTION) + 1))
		.required()
		.nargs(1)
		...
		.action([](const std::string& val_) {
			...
			auto valw = xivres::util::unicode::convert<std::wstring>(val_);
			CharLowerW(&valw[0]);
			auto val = xivres::util::unicode::convert<std::string>(valw);
	...
	argp.add_argument("--launcher", "-l")
		.help(xivres::util::unicode::convert<std::string>(FindStringResourceEx(Module(), IDS_HELP_LAUNCHER) + 1))
		.required()
		.nargs(1)
	...
	argp.add_argument("--install-mode", "-i")
		.help(xivres::util::unicode::convert<std::string>(FindStringResourceEx(Module(), IDS_HELP_INSTALL_MODE) + 1))
		.required()
		.nargs(1)
		...
		.action([](const std::string& val_) {
			...
			for (size_t i = 0; i < static_cast<size_t>(InstallMode::Count_); ++i) {
				auto compare = xivres::util::unicode::convert<std::wstring>(argparse::details::repr(static_cast<InstallMode>(i)));
				CharLowerW(&compare[0]);
			...
			throw std::runtime_error(xivres::util::unicode::convert<std::string>(FindStringResourceEx(Module(), IDS_ERROR_INVALID_INSTALL_MODE) + 1));
		});
	argp.add_argument("--quiet", "-q")
		.help(xivres::util::unicode::convert<std::string>(FindStringResourceEx(Module(), IDS_HELP_QUIET) + 1))
		.default_value(false)
		.implicit_value(true);
	argp.add_argument("--disable-runas", "-d")
		.help(xivres::util::unicode::convert<std::string>(FindStringResourceEx(Module(), IDS_HELP_DISABLE_RUNAS) + 1))
		.default_value(false)
		.implicit_value(true);

	// internal use
	argp.add_argument("--handle-instead-of-pid")
		.help(xivres::util::unicode::convert<std::string>(FindStringResourceEx(Module(), IDS_HELP_INTERNAL_USE_ONLY) + 1))
		.required()
		.default_value(false)
		.implicit_value(true);
	...
	argp.add_argument("targets")
		.help(xivres::util::unicode::convert<std::string>(FindStringResourceEx(Module(), IDS_HELP_TARGETS) + 1))
		.remaining();
}
...
void XivAlexander::LoaderApp::Arguments::Parse() {
	...
	if (int nArgs; LPWSTR* szArgList = CommandLineToArgvW(GetOriginalCommandLine().data(), &nArgs)) {
		for (int i = 0; i < nArgs; i++)
			args.emplace_back(xivres::util::unicode::convert<std::string>(szArgList[i]));
		LocalFree(szArgList);
	}
	...
	if (argp.present("targets")) {
		...
		if (argp.get<bool>("--handle-instead-of-pid")) {
			...
			if (!targets.empty()) {
				m_runProgram = xivres::util::unicode::convert<std::wstring>(targets[0]);
				m_runProgramArgs = xivres::util::unicode::convert<std::wstring>(Utils::Win32::ReverseCommandLineToArgv(std::span(targets.begin() + 1, targets.end())));
			}
			...
			for (const auto& target : targets) {
				...
				if (idx != target.length()) {
					auto buf = xivres::util::unicode::convert<std::wstring>(target);
					CharLowerW(&buf[0]);
...
std::wstring XivAlexander::LoaderApp::Arguments::GetHelpMessage() const {
	...
	return std::vformat(FindStringResourceEx(Module(), IDS_APP_DESCRIPTION) + 1, std::make_wformat_args(s));
}
