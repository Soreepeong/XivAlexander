...
namespace Utils::Crypt {
	...
	void BlowfishEcbEncrypt(std::span<const T1> key, std::span<T2> data) {
		BlowfishEcbEncrypt(xivres::util::span_cast<const uint8_t>(key), xivres::util::span_cast<uint8_t>(data));
	}
	
	template<typename T1, typename T2>
	void BlowfishEcbDecrypt(std::span<const T1> key, std::span<T2> data) {
		BlowfishEcbDecrypt(xivres::util::span_cast<const uint8_t>(key), xivres::util::span_cast<uint8_t>(data));
	}
}
