...
namespace Utils::Win32 {
	...
	public:
		...
		template<typename T>
		xivres::util::wrap_value_with_context<T*> WithVirtualAlloc(void* lpBase, DWORD flAllocType, DWORD flProtect, const std::span<T>& lpSourceData = nullptr) const {
			const auto lpAddress = VirtualAlloc(lpBase, flAllocType, flProtect, lpSourceData);
			return xivres::util::wrap_value_with_context(lpAddress, [this, lpAddress]() {
		...
		xivres::util::wrap_value_with_context<DWORD> WithVirtualProtect(void* lpBase, size_t offset, size_t length, DWORD newProtect) const;
	...
	public:
		...
		ProcessBuilder& WithStdin(Handle);
		ProcessBuilder& WithStdin(HANDLE = nullptr);
		ProcessBuilder& WithStdout(Handle);
		ProcessBuilder& WithStdout(HANDLE = nullptr);
		ProcessBuilder& WithStderr(Handle);
