...
XivAlexander::LoaderApp::Actions::RunLauncher::RunLauncher(const Arguments& args)
	: m_args(args) {
}

bool XivAlexander::LoaderApp::Actions::RunLauncher::SelectAndRunLauncher() {
	...
	try {
		...
		Utils::Win32::Error::ThrowIfFailed(pDialog->SetTitle(FindStringResourceEx(Module(), IDS_TITLE_SELECT_BOOT) + 1));
		...
		MessageBoxF(nullptr, MB_ICONERROR, IDS_ERROR_UNEXPECTED, e.what() ? e.what() : "Unknown");
	}
	return true;
}

int XivAlexander::LoaderApp::Actions::RunLauncher::Run() {
	try {
		EnableInjectOnCreateProcess(InjectOnCreateProcessAppFlags::Use | InjectOnCreateProcessAppFlags::InjectAll);

		const auto launchers = Misc::GameInstallationDetector::FindInstallations();
			...
			case LauncherType::Auto: {
				if (launchers.empty() || !m_args.m_runProgram.empty())
					...
					for (const auto& launcher : launchers) {
						if (MessageBoxF(nullptr, MB_YESNO | MB_ICONQUESTION,
							IDS_CONFIRM_LAUNCH,
		...
		if (m_args.m_action == LoaderAction::Interactive) {
			...
			MessageBoxF(nullptr, MB_OK | MB_ICONINFORMATION, IDS_ERROR_NOT_FOUND);
		}
		return -1;
