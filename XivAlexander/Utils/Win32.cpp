...
std::string Utils::Win32::FormatWindowsErrorMessage(unsigned int errorCode, int languageId) {
	...
	for (const auto langId : {
		...
		if (nullptr != errorText) {
			messages.insert(xivres::util::unicode::convert<std::string>(errorText));
			LocalFree(errorText);
		}
	}
	std::string res;
...
int Utils::Win32::MessageBoxF(HWND hWnd, UINT uType, const wchar_t* lpCaption, const std::string& text) {
	return MessageBoxF(hWnd, uType, lpCaption, xivres::util::unicode::convert<std::wstring>(text));
}
...
std::filesystem::path Utils::Win32::EnsureDirectory(const std::filesystem::path& path) {
	if (!is_directory(path)) {
		if (const auto res = SHCreateDirectoryExW(nullptr, path.c_str(), nullptr);
			...
			throw Error(res, "SHCreateDirectoryExW");
		if (!is_directory(path))
...
bool Utils::Win32::IsUserAnAdmin() {
	...
	if (!AllocateAndInitializeSid(
		...
		&adminGroup))
		return false;
	const auto cleanup = xivres::util::on_dtor([adminGroup]() { FreeSid(adminGroup); });
...
Utils::Win32::Error::Error(const _com_error& e)
	: std::runtime_error(
		std::format("[0x{:08x}] {} ({})",
			...
			xivres::util::unicode::convert<std::string>(e.ErrorMessage()),
			xivres::util::unicode::convert<std::string>(e.Description().length() ? static_cast<const wchar_t*>(e.Description()) : L""))
	)
