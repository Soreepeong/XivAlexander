...
const uint32_t Sqex::Sqpack::SqexHashTable[4][256] = {
...
bool Sqex::Sqpack::Sha1Value::operator==(const char(& r)[20]) const {
...
bool Sqex::Sqpack::Sha1Value::IsZero() const {
...
void Sqex::Sqpack::SqData::Header::Verify(uint32_t expectedSpanIndex) const {
	...
	if (MaxFileSize > MaxFileSize_MaxValue)
		throw CorruptDataException(std::format("MaxFileSize({:x}) != MaxFileSize_MaxValue({:x})", MaxFileSize.Value(), MaxFileSize_MaxValue));
