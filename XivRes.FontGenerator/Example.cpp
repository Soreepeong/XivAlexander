...
const auto fontChns = XivRes::GameReader(R"(C:\Program Files (x86)\SNDA\FFXIV\game)").GetFonts(XivRes::GameFontType::chn_axis);
const auto fontKrns = XivRes::GameReader(R"(C:\Program Files (x86)\FINAL FANTASY XIV - KOREA\game)").GetFonts(XivRes::GameFontType::krn_axis);
...
class WindowImpl {
	...
	LRESULT OnCreate() {
...

		{
			...
			col.cx = 80;
			col.pszText = const_cast<wchar_t*>(L"Δ Advance X");
			ListView_InsertColumn(m_hExtraFontsList, colIndex++, &col);

			col.cx = 80;
			col.pszText = const_cast<wchar_t*>(L"Δ Current Y");
			ListView_InsertColumn(m_hExtraFontsList, colIndex++, &col);
...
public:
	...
	void LoadFonts() {
		...
		if (fontIndex < fontGlos.size())
			m_fontBase = fontGlos[fontIndex];
		else if ((fontIndex -= fontGlos.size()) < fontChns.size())
			m_fontBase = fontChns[fontIndex];
		else if ((fontIndex -= fontChns.size()) < fontKrns.size())
			m_fontBase = fontKrns[fontIndex];
		else if ((fontIndex -= fontKrns.size()) < _countof(NoBaseFontSizes))
