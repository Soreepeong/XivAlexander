...
bool App::Misc::Signatures::SectionFilterTextOnly(const IMAGE_SECTION_HEADER& pSectionHeader) {
	return 0 == strncmp(reinterpret_cast<const char*>(pSectionHeader.Name), ".text", 6);
}

std::vector<void*> App::Misc::Signatures::LookupForData(SectionFilter lookupInSection, const char* sPattern, const char* sMask, size_t length, [[maybe_unused]] const std::vector<size_t>& nextOffsets) {
