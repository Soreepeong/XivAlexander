...
void XivAlexander::Apps::MainApp::Window::ProgressPopupWindow::UpdateProgress(uint64_t progress, uint64_t max) {
	...
	if (!max) {
		...
		if (prevStyle & PBS_MARQUEE) {
			...
			SendMessageW(m_hProgressBar, PBM_SETRANGE32, 0, 10000000);
...
void XivAlexander::Apps::MainApp::Window::ProgressPopupWindow::UpdateMessage(const std::string& message) {
	UpdateMessage(xivres::util::unicode::convert<std::wstring>(message));
}

void XivAlexander::Apps::MainApp::Window::ProgressPopupWindow::UpdateMessage(const std::wstring& message) {
	SendMessageW(m_hMessage, WM_SETTEXT, 0, reinterpret_cast<LPARAM>(message.c_str()));
}

void XivAlexander::Apps::MainApp::Window::ProgressPopupWindow::Cancel() {
	SendMessageW(m_hWnd, WM_CLOSE, 0, 0);
}

const Utils::Win32::Event& XivAlexander::Apps::MainApp::Window::ProgressPopupWindow::GetCancelEvent() const {
	return m_hCancelEvent;
}

DWORD XivAlexander::Apps::MainApp::Window::ProgressPopupWindow::DoModalLoop(int ms, std::vector<HANDLE> events) {
...
LRESULT XivAlexander::Apps::MainApp::Window::ProgressPopupWindow::WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
