...
void XivAlexander::Apps::MainApp::Window::Dialog::FramerateLockingDialog::ShowModal(Apps::MainApp::App& app, HWND hParentWindow, const Utils::Win32::Event& hCancelEvent) {
	struct Data {
		...
		Data(Apps::MainApp::App& app, HWND hwnd, HWND hParentWindow)
			: App(app)
			, Hwnd(hwnd)
		...
		bool TryRegisterTimingHandler() {
			if (auto& handler = App.GetNetworkTimingHandler()) {
				CleanupNetworkTimingHandlerCooldownCallback = handler->OnCooldownGroupUpdateListener([&](const Apps::MainApp::Internal::NetworkTimingHandler::CooldownGroup& group, bool newDriftItem) {
					if (group.Id != Apps::MainApp::Internal::NetworkTimingHandler::CooldownGroup::Id_Gcd)
						return;
				...
				if (const auto& group = handler->GetCooldownGroup(Apps::MainApp::Internal::NetworkTimingHandler::CooldownGroup::Id_Gcd); group.DurationUs != UINT64_MAX) {
...
Utils::CallOnDestruction XivAlexander::Apps::MainApp::Window::Dialog::FramerateLockingDialog::Show(Apps::MainApp::App& app, HWND hParentWindow) {
