...
struct XivAlexander::Apps::MainApp::Features::NetworkTimingHandler::Implementation {
	...
	public:
		...
		SingleConnectionHandler(Implementation* pImpl, SingleConnection& conn)
			...
			conn.AddIncomingFFXIVMessageHandler(this, [&](auto pMessage) {
				...
				if (pMessage->Type == message_type::Ipc && pMessage->Data.Ipc.Type == ipc_type::CustomType) {
					if (pMessage->Data.Ipc.SubType == static_cast<uint16_t>(ipc_custom_subtype::OriginalWaitTime)) {
				...
				} else if (pMessage->Type == message_type::Ipc && pMessage->Data.Ipc.Type == ipc_type::InterestedType) {
							...
							if (actionEffect.SourceSequence == 0) {
								...
								if (LatestSuccessfulRequest && !LatestSuccessfulRequest->CastTimeUs && LatestSuccessfulRequest->Sequence) {
									...
									*LastAnimationLockEndsAtUs += (originalWaitUs + nowUs) - (LatestSuccessfulRequest->OriginalWaitUs + LatestSuccessfulRequest->ResponseUs);
									LastAnimationLockEndsAtUs = xivres::util::clamp(*LastAnimationLockEndsAtUs, nowUs + AutoAttackDelayUs, nowUs + AutoAttackDelayUs + originalWaitUs);
								...
								if (!PendingActions.empty()) {
									...
									if (!LatestSuccessfulRequest->CastTimeUs) {
										const auto rttUs = nowUs - LatestSuccessfulRequest->RequestUs;
	...
	Implementation(NetworkTimingHandler& this_, MainApp::App& app)
		: This(this_)
		, App(app)
	...
	void CallOnActionRequestListener(const ipcs::C2S_ActionRequest& req) const {
		This.OnActionRequestListener(req);
	}
};

XivAlexander::Apps::MainApp::Features::NetworkTimingHandler::NetworkTimingHandler(App& app)
	: m_pImpl(std::make_unique<Implementation>(*this, app)) {
}

XivAlexander::Apps::MainApp::Features::NetworkTimingHandler::~NetworkTimingHandler() = default;

const XivAlexander::Apps::MainApp::Features::NetworkTimingHandler::CooldownGroup& XivAlexander::Apps::MainApp::Features::NetworkTimingHandler::GetCooldownGroup(uint32_t groupId) const {
