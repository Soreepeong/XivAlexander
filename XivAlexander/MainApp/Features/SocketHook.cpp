...
class XivAlexander::Apps::MainApp::Features::SingleConnection::SingleStream {
	Misc::Logger& m_logger;
	const std::string m_name;
	xivres::util::zlib_deflater m_deflater;
	xivres::util::zlib_inflater m_inflater;
	Game::Oodle::Oodler m_oodler, m_unoodler;
...
public:
	...
	SingleStream(Misc::Logger& logger, std::string name, const Game::Oodle::OodleModule& oodleModule, bool oodleTcp)
		: m_logger(logger)
	...
	void TunnelXivStream(SingleStream& target, const MessageMangler& messageMangler) {
		while (true) {
			auto buf = Peek();
			if (buf.empty())
				break;

			if (const auto trash = bundle::extract_front_trash(buf); !trash.empty()) {
				target.Write(trash);
			...
			// Incomplete header
			if (buf.size_bytes() < sizeof bundle_header)
				break;

			const auto* pGamePacket = reinterpret_cast<const bundle*>(buf.data());
			...
			try {
				auto messages = pGamePacket->get_messages(
					[this](std::span<const uint8_t> src) { return m_inflater(src); },
					[this](std::span<const uint8_t> src, size_t decodedLength) { return m_unoodler.Decode(src, decodedLength); });
				auto header = *pGamePacket;
				header.TotalLength = static_cast<uint32_t>(sizeof bundle_header);
				header.MessageCount = 0;
				header.CompressionType = pGamePacket->CompressionType;
				header.DecodedBodyLength = 0;

				for (auto& msg : messages) {
					const auto pMessage = reinterpret_cast<message*>(&msg[0]);
				...
				for (const auto& msg : messages) {
					if (!reinterpret_cast<const message*>(&msg[0])->Length)
						continue;
				...
				target.Write(&header, sizeof bundle_header);
				target.Write(encoded);
			} catch (const std::exception& e) {
				m_logger.Format<LogLevel::Warning>(LogCategory::SocketHook, "{}: Error: {}\n{}", m_name, e.what(), pGamePacket->represent());
...
struct XivAlexander::Apps::MainApp::Features::SingleConnection::Implementation {
	...
	xivres::util::on_dtor PingTrackKeeper;

	mutable int IoctlTcpInfoFailureCount = 0;
	uint64_t NextTcpDelaySetAttempt = 0;

	Implementation(Features::SingleConnection& singleConnection, Features::SocketHook& socketHook);
	...
	void ProcessRecvData() {
		RecvRaw.TunnelXivStream(RecvProcessed, [&](auto* pMessage) {
			...
			switch (pMessage->Type) {
				case message_type::ServerKeepAlive:
	...
	void ProcessSendData() {
		SendRaw.TunnelXivStream(SendProcessed, [&](auto* pMessage) {
			...
			switch (pMessage->Type) {
				case message_type::ClientKeepAlive:
...
struct XivAlexander::Apps::MainApp::Features::SocketHook::Implementation {
	...
	Game::Oodle::OodleModule OodleModule;

	Implementation(Features::SocketHook& socketHook, MainApp::App& app)
...
void XivAlexander::Apps::MainApp::Features::SingleConnection::Implementation::ResolveAddresses() {
...
XivAlexander::Apps::MainApp::Features::SingleConnection::Implementation::Implementation(Features::SingleConnection& singleConnection, Features::SocketHook& socketHook)
	: SingleConnection(singleConnection)
	, SocketHook(socketHook)
...
XivAlexander::Apps::MainApp::Features::SingleConnection::SingleConnection(SocketHook& hook, SOCKET s)
	: m_socket(s)
	, m_pImpl(std::make_unique<Implementation>(*this, hook)) {
}

XivAlexander::Apps::MainApp::Features::SingleConnection::~SingleConnection() = default;

void XivAlexander::Apps::MainApp::Features::SingleConnection::AddIncomingFFXIVMessageHandler(void* token, MessageMangler cb) {
	m_pImpl->IncomingHandlers[reinterpret_cast<size_t>(token)].emplace_back(std::move(cb));
}

void XivAlexander::Apps::MainApp::Features::SingleConnection::AddOutgoingFFXIVMessageHandler(void* token, MessageMangler cb) {
	m_pImpl->OutgoingHandlers[reinterpret_cast<size_t>(token)].emplace_back(std::move(cb));
}

void XivAlexander::Apps::MainApp::Features::SingleConnection::RemoveMessageHandlers(void* token) {
	m_pImpl->IncomingHandlers.erase(reinterpret_cast<size_t>(token));
	m_pImpl->OutgoingHandlers.erase(reinterpret_cast<size_t>(token));
}

void XivAlexander::Apps::MainApp::Features::SingleConnection::ResolveAddresses() {
	m_pImpl->ResolveAddresses();
}

std::optional<int64_t> XivAlexander::Apps::MainApp::Features::SingleConnection::FetchSocketLatencyUs() {
...
const Utils::NumericStatisticsTracker* XivAlexander::Apps::MainApp::Features::SingleConnection::GetPingLatencyTrackerUs() const {
...
XivAlexander::Apps::MainApp::Features::SocketHook::SocketHook(App & app)
	...
	m_hThreadSetupHook = Utils::Win32::Thread(L"SocketHook::SocketHook/WaitGameWindow", [this, &app]() {
		...
		app.RunOnGameLoop([&]() {
			...
			Game::Oodle::Oodler(m_pImpl->OodleModule, false);
...
XivAlexander::Apps::MainApp::Features::SocketHook::~SocketHook() {
	m_unloading = true;
...
bool XivAlexander::Apps::MainApp::Features::SocketHook::IsUnloadable() const {
...
void XivAlexander::Apps::MainApp::Features::SocketHook::ReleaseSockets() const {
	if (!m_pImpl)
		return;
...
void XivAlexander::Apps::MainApp::Features::SocketHook::ResetAllConnections() const {
	if (!m_pImpl)
		return;
...
std::wstring XivAlexander::Apps::MainApp::Features::SocketHook::Describe() const {
	if (!m_pImpl)
		return {};
		...
		try {
			...
			for (const auto& entry : m_pImpl->Sockets) {
				...
				result += m_pImpl->Config->Runtime.FormatStringRes(IDS_SOCKETHOOK_SOCKET_DESCRIBE_TITLE,
					entry.first,
					xivres::util::unicode::convert<std::wstring>(Utils::ToString(conn->m_pImpl->LocalAddress)),
					xivres::util::unicode::convert<std::wstring>(Utils::ToString(conn->m_pImpl->RemoteAddress)));
