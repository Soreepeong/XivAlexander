...
Utils::Crypt::Sha1::Sha1()
	: m_hAlg(nullptr)
	, m_hHash(nullptr) {
	CheckNtStatus(BCryptOpenAlgorithmProvider(&m_hAlg, BCRYPT_SHA1_ALGORITHM, nullptr, 0), "BCryptOpenAlgorithmProvider(SHA1)");
	try {
		...
		CheckNtStatus(BCryptCreateHash(m_hAlg, &m_hHash, m_hashObject.data(), hashObjectSize, nullptr, 0, 0), "BCryptCreateHash(SHA1)");
	} catch (...) {
...
namespace {
	...
	constexpr uint32_t BlowfishS[4][256] = {
		{
	...
	struct BlowfishContext {
		...
		BlowfishContext(std::span<const uint8_t> key) {
			...
			for (int i = 0, j = 0; i < 18; ++i) {
				...
				for (int k = 0; k < 4; ++k) {
					data = (data << 8) | key[j];
					j = (j + 1) % static_cast<int>(key.size());
				}
				P[i] ^= data;
			}
		...
		static uint32_t Feistel(const uint32_t S[4][256], uint32_t x) {
...
void Utils::Crypt::BlowfishEcbEncrypt(std::span<const uint8_t> key, std::span<uint8_t> data) {
	if (data.size() % 8 != 0)
		throw std::invalid_argument("Blowfish ECB data length must be a multiple of 8");

	const BlowfishContext ctx(key);

	for (size_t i = 0; i < data.size(); i += 8) {
		uint32_t L = (data[i] << 24) | (data[i + 1] << 16) | (data[i + 2] << 8) | data[i + 3];
		uint32_t R = (data[i + 4] << 24) | (data[i + 5] << 16) | (data[i + 6] << 8) | data[i + 7];
		ctx.EncryptBlock(L, R);
		data[i] = (L >> 24) & 0xFF;
		data[i + 1] = (L >> 16) & 0xFF;
		data[i + 2] = (L >> 8) & 0xFF;
		data[i + 3] = L & 0xFF;
...
void Utils::Crypt::BlowfishEcbDecrypt(std::span<const uint8_t> key, std::span<uint8_t> data) {
	if (data.size() % 8 != 0)
		throw std::invalid_argument("Blowfish ECB data length must be a multiple of 8");
